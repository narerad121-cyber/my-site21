...
let isFrontCamera = false;  // ตัวแปรเช็คกล้องหน้า
    ...
    if (qrReaderContainer.style.display === 'none') {
        ...
        try {
            ...
            if (cameras.length > 0) {
                ...
                // เช็คกล้องที่ใช้อยู่ และสลับระหว่างกล้องหน้า/หลัง
                if (isFrontCamera) {
                    cameraId = cameras.find(cam => cam.label.toLowerCase().includes('back') || cam.label.toLowerCase().includes('rear'))?.id || cameras[0]?.id;
                } else {
                    cameraId = cameras.find(cam => cam.label.toLowerCase().includes('front') || cam.label.toLowerCase().includes('face'))?.id || cameras[0]?.id;
                }

                // สตาร์ทการสแกน QR code
                if (cameraId) {
...
switchCameraBtn.addEventListener('click', () => {
    isFrontCamera = !isFrontCamera;  // สลับค่า
    // ปิดการสแกนแล้วสตาร์ทใหม่
    html5QrCodeScanner.stop().then(() => {
        qrReaderContainer.style.display = 'none';
        scanQrBtn.click(); // กดปุ่ม scanQrBtn ใหม่เพื่อเริ่มกล้องใหม่
    });
});
